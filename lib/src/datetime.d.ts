export declare const datetime: {
    isLeap: (year: string | number) => boolean;
    microsecondsInMillisecond: number;
    microsecondsPerMillisecond: number;
    microsecondsInMs: number;
    microsecondsPerMs: number;
    usInMs: number;
    usPerMs: number;
    usecondsInMs: number;
    usecondsPerMs: number;
    usInMillisecond: number;
    usPerMillisecond: number;
    usecondInMillisecond: number;
    usecondPerMillisecond: number;
    usecondsInMillisecond: number;
    usecondsPerMillisecond: number;
    μInMs: number;
    μPerMs: number;
    μsInMs: number;
    μsPerMs: number;
    μsecondsInMs: number;
    μsecondsPerMs: number;
    μInMillisecond: number;
    μPerMillisecond: number;
    μsecondInMillisecond: number;
    μsecondPerMillisecond: number;
    μsInMillisecond: number;
    μsPerMillisecond: number;
    μsecondsInMillisecond: number;
    μsecondsPerMillisecond: number;
    msInSec: number;
    msPerSec: number;
    millisecondsInSeconds: number;
    millisecondsInSecond: number;
    millisecondsPerSecond: number;
    msInSeconds: number;
    msInSecond: number;
    msInASecond: number;
    msIn1Sec: number;
    msPerSecond: number;
    msPerSeconds: number;
    millisecondsPerSec: number;
    secInMin: number;
    secsInMin: number;
    secPerMin: number;
    secsPerMin: number;
    secondsPerMinute: number;
    secondsInMinute: number;
    secPerMinute: number;
    secInMinute: number;
    secsPerMinute: number;
    secsInMinute: number;
    secInHr: number;
    secsInHr: number;
    secPerHr: number;
    secsPerHr: number;
    secPerHour: number;
    secInHour: number;
    secsPerHour: number;
    secsInHour: number;
    secondsPerHour: number;
    secondsInHour: number;
    secondsPerHr: number;
    secondsInHr: number;
    secInDay: number;
    secsInDay: number;
    secPerDay: number;
    secsPerDay: number;
    secondInDay: number;
    secondsInDay: number;
    secondPerDay: number;
    secondsPerDay: number;
    minInHr: number;
    minInHour: number;
    minutesInHr: number;
    minutesInHour: number;
    minPerHr: number;
    minPerHour: number;
    minutesPerHr: number;
    minutesPerHour: number;
    hrInDay: number;
    hrsInDay: number;
    hoursInDay: number;
    hrPerDay: number;
    hrsPerDay: number;
    hoursPerDay: number;
    daysInCommonYear: number;
    daysInLeapYear: number;
    daysPerCommonYear: number;
    daysPerLeapYear: number;
    daysInCommonYr: number;
    daysInLeapYr: number;
    daysPerCommonYr: number;
    daysPerLeapYr: number;
    daysPerMonth: {
        jan: number;
        january: number;
        feb: {
            common: number;
            leap: number;
        };
        february: {
            common: number;
            leap: number;
        };
        mar: number;
        march: number;
        apr: number;
        april: number;
        may: number;
        jun: number;
        june: number;
        jul: number;
        july: number;
        aug: number;
        august: number;
        sep: number;
        sept: number;
        september: number;
        oct: number;
        october: number;
        nov: number;
        november: number;
        dec: number;
        december: number;
    };
    daysInMonth: {
        jan: number;
        january: number;
        feb: {
            common: number;
            leap: number;
        };
        february: {
            common: number;
            leap: number;
        };
        mar: number;
        march: number;
        apr: number;
        april: number;
        may: number;
        jun: number;
        june: number;
        jul: number;
        july: number;
        aug: number;
        august: number;
        sep: number;
        sept: number;
        september: number;
        oct: number;
        october: number;
        nov: number;
        november: number;
        dec: number;
        december: number;
    };
    daysPerMon: {
        jan: number;
        january: number;
        feb: {
            common: number;
            leap: number;
        };
        february: {
            common: number;
            leap: number;
        };
        mar: number;
        march: number;
        apr: number;
        april: number;
        may: number;
        jun: number;
        june: number;
        jul: number;
        july: number;
        aug: number;
        august: number;
        sep: number;
        sept: number;
        september: number;
        oct: number;
        october: number;
        nov: number;
        november: number;
        dec: number;
        december: number;
    };
    daysInMon: {
        jan: number;
        january: number;
        feb: {
            common: number;
            leap: number;
        };
        february: {
            common: number;
            leap: number;
        };
        mar: number;
        march: number;
        apr: number;
        april: number;
        may: number;
        jun: number;
        june: number;
        jul: number;
        july: number;
        aug: number;
        august: number;
        sep: number;
        sept: number;
        september: number;
        oct: number;
        october: number;
        nov: number;
        november: number;
        dec: number;
        december: number;
    };
};
